...
public class MobSpawn {
    ...
    public static void onEntityJoinWorld(EntityJoinWorldEvent event) {
        ...
        if (entity.isCreatureType(EnumCreatureType.MONSTER, false)) {
            ...
            Main.LOGGER.info("Monster Spawned: " + entity + " count = " + entityCount);
            // If the entity count exceeds the limit, cancel the spawn event
            if (entityCount >= settings.MONSTER_LIMIT_NUMBER) {
                event.setCanceled(true);
                Main.LOGGER.info("Monster Cannot Spawned: " + entity + " because of " + entityCount + " entities in the chunk");
            }
        }
    }
}
